...
<div class="container-fluid">
	<div class="col-lg-12">
		<div class="card">
			<div class="card-header">Permission Request  List</div>
			<div class="card-body">
				<table class="table table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<p>Student: <b><?php echo ucwords($row['name']). " (".$row['eID'].")" ?></b></p>
								<p>Permission Type: <b><?php echo $lt[$row['leave_type_id']] ?></b></p>
...
<script>
	...
	$('.approved_leave').click(function(){
		_conf("Are you sure to this Permission Request ?","action_leave",[$(this).attr('data-id'),1])
		
	})
	$('.decline_leave').click(function(){
		_conf("Are you sure to decline this Permission Request ?","action_leave",[$(this).attr('data-id'),2])
	})
	function action_leave($id,$status){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Permission Request  succesffuly updated",'success')
					setTimeout(function(){
						location.reload()
					},1500)
