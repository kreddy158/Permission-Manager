	...
	<div class="col-lg-12">
		<form action="" id="manage-leave">
			...
			<div class="form-group">
				<label for="" class="control-label">Permission Type</label>
				...
				<small><i>Your available credits for selected Permission Type id: <span id="credits">0</span></i></small>
			</div>
...
<script>
	...
	$('#leave_type_id').change(function(){
		...
		$.ajax({
			...
			success:function(resp){
					$('#credits').html(resp)
					if(resp <=  1){
						$('#leave_type_id').closest('.form-group').append('<div class="alert alert-danger err-msg">You dont have an available credits with the selected Permission Type.</div>')
						$('.on-ltchange').hide()
					}else{
						$('.on-ltchange').show()
					}
	...
	$('#manage-leave').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast("Permission Request  successfully saved",'success');
					setTimeout(function(){
						location.reload()
					},1000)
				}
